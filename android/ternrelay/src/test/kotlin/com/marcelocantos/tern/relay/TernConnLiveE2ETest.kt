 ...
 *
 * Uses tern-bridge (Go binary) for QUIC connectivity. The bridge
...
class TernConnLiveE2ETest {
    ...
    private fun buildBridge() {
        val build = ProcessBuilder("go", "build", "-o", "/tmp/tern-bridge-kt", "./cmd/tern-bridge")
    ...
    private fun startBridge(vararg args: String): Process {
        return ProcessBuilder("/tmp/tern-bridge-kt", *args)
