...
export class ServerMachine {
    ...
    codeAttempts: string = 0; // failed code submission attempts
    ...
    handleEvent(ev: EventID): CmdID[] {
        switch (true) {
            case this.state === ServerState.Idle && ev === EventID.RecvPairBegin: {
            ...
            case this.state === ServerState.GenerateToken && ev === EventID.TokenCreated: {
                this.actions.get(ActionID.RegisterRelay)?.();
                this.state = ServerState.RegisterRelay;
                return [];
            }
            case this.state === ServerState.RegisterRelay && ev === EventID.RelayRegistered: {
                this.state = ServerState.WaitingForClient;
                return [];
            }
            case this.state === ServerState.WaitingForClient && ev === EventID.RecvPairHello && this.guards.get(GuardID.TokenValid)?.() === true: {
            ...
            case this.state === ServerState.WaitingForClient && ev === EventID.RecvPairHello && this.guards.get(GuardID.TokenInvalid)?.() === true: {
                this.state = ServerState.Idle;
                return [];
            }
            case this.state === ServerState.DeriveSecret && ev === EventID.ECDHComplete: {
                this.state = ServerState.SendAck;
                return [];
            }
            case this.state === ServerState.SendAck && ev === EventID.SignalCodeDisplay: {
                this.state = ServerState.WaitingForCode;
                return [];
            }
            case this.state === ServerState.WaitingForCode && ev === EventID.RecvCodeSubmit: {
                // received_code: recv_msg.code (set by action)
                this.state = ServerState.ValidateCode;
                return [];
            }
            case this.state === ServerState.ValidateCode && ev === EventID.CheckCode && this.guards.get(GuardID.CodeCorrect)?.() === true: {
                this.state = ServerState.StorePaired;
                return [];
            }
            case this.state === ServerState.ValidateCode && ev === EventID.CheckCode && this.guards.get(GuardID.CodeWrong)?.() === true: {
                // code_attempts: code_attempts + 1 (set by action)
                this.state = ServerState.Idle;
                return [];
            }
            case this.state === ServerState.StorePaired && ev === EventID.Finalise: {
            ...
            case this.state === ServerState.Paired && ev === EventID.RecvAuthRequest: {
            ...
            case this.state === ServerState.AuthCheck && ev === EventID.Verify && this.guards.get(GuardID.DeviceKnown)?.() === true: {
            ...
            case this.state === ServerState.AuthCheck && ev === EventID.Verify && this.guards.get(GuardID.DeviceUnknown)?.() === true: {
                this.state = ServerState.Idle;
                return [];
            }
            case this.state === ServerState.SessionActive && ev === EventID.Disconnect: {
                this.state = ServerState.Paired;
                return [];
            }
        }
...
export class IosMachine {
    ...
    handleEvent(ev: EventID): CmdID[] {
        switch (true) {
            case this.state === IosState.Idle && ev === EventID.UserScansQR: {
                this.state = IosState.ScanQR;
                return [];
            }
            case this.state === IosState.ScanQR && ev === EventID.QRParsed: {
                this.state = IosState.ConnectRelay;
                return [];
            }
            case this.state === IosState.ConnectRelay && ev === EventID.RelayConnected: {
                this.state = IosState.GenKeyPair;
                return [];
            }
            case this.state === IosState.GenKeyPair && ev === EventID.KeyPairGenerated: {
                this.actions.get(ActionID.SendPairHello)?.();
                this.state = IosState.WaitAck;
                return [];
            }
            case this.state === IosState.WaitAck && ev === EventID.RecvPairHelloAck: {
                this.actions.get(ActionID.DeriveSecret)?.();
            ...
            case this.state === IosState.E2EReady && ev === EventID.RecvPairConfirm: {
                // ios_code: DeriveCode(received_server_pub, "client_pub") (set by action)
                this.state = IosState.ShowCode;
                return [];
            }
            case this.state === IosState.ShowCode && ev === EventID.CodeDisplayed: {
                this.state = IosState.WaitPairComplete;
                return [];
            }
            case this.state === IosState.WaitPairComplete && ev === EventID.RecvPairComplete: {
                this.actions.get(ActionID.StoreSecret)?.();
                this.state = IosState.Paired;
                return [];
            }
            case this.state === IosState.Paired && ev === EventID.AppLaunch: {
                this.state = IosState.Reconnect;
                return [];
            }
            case this.state === IosState.Reconnect && ev === EventID.RelayConnected: {
                this.state = IosState.SendAuth;
                return [];
            }
            case this.state === IosState.SendAuth && ev === EventID.RecvAuthOk: {
                this.state = IosState.SessionActive;
                return [];
            }
            case this.state === IosState.SessionActive && ev === EventID.Disconnect: {
                this.state = IosState.Paired;
                return [];
            }
        }
...
export class CliMachine {
    ...
    handleEvent(ev: EventID): CmdID[] {
        switch (true) {
            case this.state === CliState.Idle && ev === EventID.CliInit: {
                this.state = CliState.GetKey;
                return [];
            }
            case this.state === CliState.GetKey && ev === EventID.KeyStored: {
                this.state = CliState.BeginPair;
                return [];
            }
            case this.state === CliState.BeginPair && ev === EventID.RecvTokenResponse: {
                this.state = CliState.ShowQR;
                return [];
            }
            case this.state === CliState.ShowQR && ev === EventID.RecvWaitingForCode: {
                this.state = CliState.PromptCode;
                return [];
            }
            case this.state === CliState.PromptCode && ev === EventID.UserEntersCode: {
                this.state = CliState.SubmitCode;
                return [];
            }
            case this.state === CliState.SubmitCode && ev === EventID.RecvPairStatus: {
